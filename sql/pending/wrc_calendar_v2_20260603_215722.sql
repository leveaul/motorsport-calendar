...
SELECT 'WRC', v.name, v.circuit, v.country, v.ds::date, v.de::date, v.rnd, 'race',
  CASE WHEN v.ds::date < CURRENT_DATE THEN 'done' ELSE 'upcoming' END,
  v.ckey
FROM (VALUES
  (1, 'Rallye Monte-Carlo',      'Monaco / Alpes françaises', 'Monaco',         '2026-01-22','2026-01-25','wrc_montecarlo'),
  (2, 'Rally Sweden',            'Umeå',                      'Suède',           '2026-02-12','2026-02-15','wrc_sweden'),
  ...
  (8, 'EKO Acropolis Rally Greece','Lamia',                   'Grèce',           '2026-06-25','2026-06-28','wrc_greece'),
  (9, 'Rally Estonia',           'Tartu',                     'Estonie',         '2026-07-16','2026-07-19','wrc_estonia'),
  (10,'Rally Finland',           'Jyväskylä',                 'Finlande',        '2026-07-30','2026-08-02','wrc_finland'),
...
WHERE NOT EXISTS (
  SELECT 1 FROM races WHERE series_id='WRC' AND round=v.rnd
);
