...
-- ── Fix dates sprint F1 ───────────────────────────────────────────────
-- Sprint = samedi, Course = dimanche → sprint.date_start = course.date_start - 1 jour
UPDATE races AS s
...
SELECT 'circuits insérés' AS status, count(*) FROM circuits;
SELECT round, type, name, date_start FROM races
